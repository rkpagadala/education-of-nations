...
Note on paper values: the paper draft stated edu R²=0.369/0.194/0.112 at lags 25/50/75
and GDP R²=0.126/0.063/0.013. These were not verified by running the script. The values
above are the verified output of this script.
...
OUT         = os.path.join(REPO_ROOT, "paper", "fig_lag_comparison.png")
...
def within_r2(predictor_col, outcome_col, rows):
    ...
    return reg.score(p[[predictor_col]], p[outcome_col])
...
ax.set_title(
    "Figure 3. Education vs. Income Predictive Power Across Lag Lengths\n"
...
checkin = {
    "script": "scripts/fig_a1_lag_decay.py",
...
checkin_path = os.path.join(CHECKIN_DIR, "fig_a1_lag_decay.json")
