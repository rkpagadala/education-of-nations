...
Usage:
    python scripts/verify_china_cr.py
"""
...
def main():
    ...
    print("\n--- Provision discontinuity (1965, lower_sec ~30%) ---")
    comp_1965 = comp[comp["year"] == 1965][["country", "lower_sec"]].copy()
    # Filter to countries with lower_sec between 20% and 40% in 1965
    peers_1965 = comp_1965[
        (comp_1965["lower_sec"] >= 20) & (comp_1965["lower_sec"] <= 40)
    ].copy()

    # Exclude aggregates (Africa, Asia, etc.)
    ...
    peers_1965 = peers_1965[~peers_1965["country"].isin(aggregates)]
    ...
    print(f"  Countries at 20-40% lower_sec in 1965: {len(peers_1965)}")
    for _, row in peers_1965.sort_values("lower_sec").iterrows():
    ...
    results["provision_discontinuity"] = {
        ...
        "expected_china_le": 53,
        "expected_peer_avg": 57.4,
        "expected_peer_range": [54, 65],
    }

    disc_pass = True
    if china_le_1965 is not None:
        if abs(china_le_1965 - 53) > 3:
            disc_pass = False
            print(f"  NOTE: China LE 1965 = {china_le_1965:.1f}, expected ~53")
    if peer_avg_le is not None:
        if abs(peer_avg_le - 57.4) > 3:
            disc_pass = False
            print(f"  NOTE: Peer avg LE = {peer_avg_le:.1f}, expected ~57.4")
    ...
    checkin_path = os.path.join(CHECKIN, "china_cr.json")
    checkin = {
        "script": "scripts/verify_china_cr.py",
        "results": results,
        "overall": overall,
    }
