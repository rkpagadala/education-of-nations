...
from _shared import PROC, REGIONS, CHECKIN, REPO_ROOT, load_wb, NAME_MAP
...
for outcome, label in [("le_2020", "Life expectancy 2020"),
    ...
    # Religion only
    X_r = sub[["protestant_colonizer"]].values
    m_r = LinearRegression().fit(X_r, y)

    # Polity2 only
    X_p = sub[["polity2_2015"]].values
    m_p = LinearRegression().fit(X_p, y)

    # Edu 1950 only
    X_e = sub[["edu_1950"]].values
    m_e = LinearRegression().fit(X_e, y)

    # All three
    X_all = sub[["edu_1950", "polity2_2015", "protestant_colonizer"]].values
    m_all = LinearRegression().fit(X_all, y)

    print(f"    Religion only:     R² = {m_r.score(X_r, y):.3f}")
    print(f"    Polity2 only:      R² = {m_p.score(X_p, y):.3f}")
    print(f"    Education only:    R² = {m_e.score(X_e, y):.3f}")
    print(f"    All three:         R² = {m_all.score(X_all, y):.3f}")
    print(f"      edu={m_all.coef_[0]:+.4f}, polity2={m_all.coef_[1]:+.4f}, "
          f"protestant={m_all.coef_[2]:+.3f}")
...
checkin = {
    "script": "colonial_education_vs_institutions.py",
...
with open(os.path.join(CHECKIN, "colonial_education_vs_institutions.json"), "w") as f:
    json.dump(checkin, f, indent=2)
    f.write("\n")
print(f"Checkin: {os.path.join(CHECKIN, 'colonial_education_vs_institutions.json')}")
