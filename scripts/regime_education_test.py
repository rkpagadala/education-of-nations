...
from _shared import PROC, REGIONS, CHECKIN, REPO_ROOT
...
def run_tests(gains_df, lag_label):
    ...
    X1 = reg[["polity2"]].values
    m1 = LinearRegression().fit(X1, y)
    r2_p = m1.score(X1, y)
    print(f"    Polity2 only:       R² = {r2_p:.4f}, coef = {m1.coef_[0]:+.3f}")

    X2 = reg[["edu_start"]].values
    m2 = LinearRegression().fit(X2, y)
    r2_s = m2.score(X2, y)
    print(f"    Starting edu only:  R² = {r2_s:.4f}, coef = {m2.coef_[0]:+.4f}")

    X3 = reg[["polity2", "edu_start"]].values
    m3 = LinearRegression().fit(X3, y)
    r2_b = m3.score(X3, y)
    print(f"    Both:               R² = {r2_b:.4f}, polity2={m3.coef_[0]:+.3f}, "
          f"start={m3.coef_[1]:+.4f}")

    return {
        "r2_polity": round(r2_p, 4),
        "r2_starting": round(r2_s, 4),
        "r2_both": round(r2_b, 4),
        "polity_coef": round(float(m1.coef_[0]), 3),
...
checkin = {
    "script": "regime_education_test.py",
...
with open(os.path.join(CHECKIN, "regime_education_test.json"), "w") as f:
    json.dump(checkin, f, indent=2)
    f.write("\n")
print(f"Checkin: {os.path.join(CHECKIN, 'regime_education_test.json')}")
