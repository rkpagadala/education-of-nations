...
for level_name, col_name in EDU_LEVELS.items():
    ...
    for ceiling in CEILINGS:
        ...
        for threshold in range(10, 91):
            ...
            if resid_result is not None:
                ...
                r2_resid, n_resid, c_resid = fe_r2("gdp_resid", "le_tp25", sub_resid)
            else:
    ...
    print(f"  {'Ceiling':<10} {'Edu→LE':>7} {'GDP→LE':>7} {'Resid→LE':>9} {'Edu→GDP':>8}")
    print(f"  {'-' * 45}")
    for ceiling in CEILINGS:
...
checkin = {
    "script": "scripts/edu_vs_gdp_residualized.py",
...
out_path = os.path.join(CHECKIN, "edu_vs_gdp_residualized.json")
