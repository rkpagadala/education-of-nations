...
# Build LE panel (same as regression_tables.py)
...
for ceiling in [60, 90]:
    ...
    n_fe = ok.sum()
    correction = (G / (G - 1)) * ((n_fe - 1) / (n_fe - 1))
    ...
    if n_ah >= 10 and n_c_ah >= 3:
        ...
        correction_ah = (G_ah / (G_ah - 1)) * ((n_ah - 1) / (n_ah - 1))
...
for ceiling in [60, 90]:
    ...
    reg_lin = LinearRegression(fit_intercept=False).fit(X_lin, gdp_dm)
    resid_lin = gdp_dm - reg_lin.predict(X_lin).ravel()
    r2_stage1_lin = reg_lin.score(X_lin, gdp_dm)

    # Second stage: resid → LE
    X_resid_lin = resid_lin.reshape(-1, 1)
    reg_s2_lin = LinearRegression(fit_intercept=False).fit(X_resid_lin, le_dm)
    r2_resid_lin = reg_s2_lin.score(X_resid_lin, le_dm)
    ...
    reg_quad = LinearRegression(fit_intercept=False).fit(X_quad, gdp_dm)
    resid_quad = gdp_dm - reg_quad.predict(X_quad).ravel()
    r2_stage1_quad = reg_quad.score(X_quad, gdp_dm)

    # Second stage: resid → LE
    X_resid_quad = resid_quad.reshape(-1, 1)
    reg_s2_quad = LinearRegression(fit_intercept=False).fit(X_resid_quad, le_dm)
    r2_resid_quad = reg_s2_quad.score(X_resid_quad, le_dm)

    # Also get education R² for LE (for comparison)
    reg_edu_le = LinearRegression(fit_intercept=False).fit(edu_dm.reshape(-1, 1), le_dm)
    r2_edu_le = reg_edu_le.score(edu_dm.reshape(-1, 1), le_dm)
...
checkin = {
    "script": "scripts/robustness_tests.py",
    "produced": pd.Timestamp.now().strftime("%Y-%m-%d"),
    "numbers": _checkin_numbers,
}
checkin_path = os.path.join(CHECKIN_DIR, "robustness_tests.json")
with open(checkin_path, "w") as f:
    json.dump(checkin, f, indent=2)
out(f"Checkin written to {checkin_path}")
