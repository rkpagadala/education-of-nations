...
Usage:
    python scripts/beta_by_baseline_group.py
"""
...
NON_SOV = [
    ...
    "Western Africa", "Eastern Asia", "South-Central Asia", "South-Eastern Asia",
...
def fe_regression(panel, x_col="parent", y_col="child"):
    """Country fixed-effects OLS via demeaning."""
    d = panel.dropna(subset=[x_col, y_col]).copy()
    if len(d) < 10:
        return None
    d["x_dm"] = d.groupby("country")[x_col].transform(lambda s: s - s.mean())
    d["y_dm"] = d.groupby("country")[y_col].transform(lambda s: s - s.mean())
    X = d["x_dm"].values
    y = d["y_dm"].values
    ss_xx = (X * X).sum()
    if ss_xx == 0:
        return None
    beta = (X * y).sum() / ss_xx
    ss_res = ((y - beta * X) ** 2).sum()
    ss_tot = (y * y).sum()
    r2 = 1 - ss_res / ss_tot if ss_tot > 0 else 0
    n_countries = d["country"].nunique()
    return {
        "beta": beta, "r2": r2, "n_obs": len(d),
...
def main():
    ...
    pooled = fe_regression(panel)
    print(f"\n  Pooled:  β={pooled['beta']:.3f}  R²={pooled['r2']:.3f}  "
          f"[N={pooled['n_obs']}, {pooled['n_countries']} countries]")
    ...
    for grp in ["Low (<20%)", "Medium (20-60%)", "High (>60%)"]:
        ...
        print(f"  {grp:<20s}  {res['beta']:8.3f}  {res['r2']:8.3f}  "
              f"{res['n_obs']:6d}  {res['n_countries']:10d}  "
...
""")
    ...
    for grp, prefix in [("Low (<20%)", "Grp-low"), ("Medium (20-60%)", "Grp-med"), ("High (>60%)", "Grp-high")]:
        ...
        res = fe_regression(sub)
        if res is not None:
            grp_numbers[f"{prefix}-beta"] = round(res["beta"], 3)
            grp_numbers[f"{prefix}-R2"] = round(res["r2"], 3)
            grp_numbers[f"{prefix}-n"] = res["n_obs"]
            grp_numbers[f"{prefix}-countries"] = res["n_countries"]

    checkin = {
        "script": "scripts/beta_by_baseline_group.py",
        "produced": pd.Timestamp.now().strftime("%Y-%m-%d"),
        "numbers": grp_numbers,
    }
    checkin_path = os.path.join(CHECKIN_DIR, "beta_by_baseline_group.json")
