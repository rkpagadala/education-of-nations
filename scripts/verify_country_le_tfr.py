# =============================================================================
# PAPER REFERENCE
# Script:  scripts/verify_country_le_tfr.py
...
def load_wb(filename):
    """Load a World Bank CSV with Country + year columns."""
    raw = pd.read_csv(os.path.join(DATA, filename))
    raw.rename(columns={raw.columns[0]: "Country"}, inplace=True)
    raw["Country"] = raw["Country"].str.strip().str.strip('"')
    ...
    long["country_lc"] = long["Country"].str.lower()
    return long

le_wb = load_wb("life_expectancy_years.csv")
tfr_wb = load_wb("children_per_woman_total_fertility.csv")
...
# ── WCDE-to-WB country name mapping ─────────────────────────────────────────
NAME_MAP = {
...
CLAIMS = [
    ...
    ("Uganda Table4 TFR=5.25", "wb_tfr", "uganda", 2020, 5.25, 0.5),
    ("Uganda Table4 LE=63.8",  "wb_le",  "uganda", 2020, 63.8, 2.0),
]
...
out_path = os.path.join(CHECKIN, "country_le_tfr.json")
