...
def run_analysis(panel, outcome_col, label):
    """Run the full analysis for one outcome."""
...
run_analysis(panel_u5, "mortality", "U5MR (UNDER-5 MORTALITY) BY EDUCATION BAND")
run_analysis(panel_u1, "mortality", "U1MR (INFANT MORTALITY) BY EDUCATION BAND")

# ── Checkin ────────────────────────────────────────────────────────

checkin = {
    "script": "scripts/u5mr_by_edu_level.py",
    "produced": pd.Timestamp.now().strftime("%Y-%m-%d"),
}

os.makedirs(CHECKIN, exist_ok=True)
out_path = os.path.join(CHECKIN, "u5mr_by_edu_level.json")
