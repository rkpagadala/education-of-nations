...
# WCDE region/aggregate entries to exclude (not sovereign states)
NON_SOVEREIGN = [
...
fem = pd.read_csv(os.path.join(PROC_DIR, "lower_sec_female.csv"), index_col="country")
...
def fe_regression(df, x_cols, y_col, cluster_col="country"):
    ...
    Returns statsmodels OLS result, N observations, N countries.
    """
    d = df.dropna(subset=x_cols + [y_col]).copy()

    # Demean within country (absorb country FE)
...
checkin = {
    "script": "scripts/table_1_main.py",
...
checkin_path = os.path.join(CHECKIN_DIR, "table_1_main.json")
